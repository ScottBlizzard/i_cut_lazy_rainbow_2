...
Frozen portfolio transfers & Holdout empty Hammer: 30/432; best singleton: 54/432 & Frozen K=4: \best{67/432}; tested oracle: 68/432 & prospective check \\
...
Table~\ref{tab:fresh-holdout} freezes the retrieved-only four-action subset before evaluating two fresh disjoint folds. The frozen policy is \texttt{aesop\_core\_plus\_learned16}, \texttt{hammerCore\_core\_plus\_learned}, \texttt{aesop\_core\_plus\_learned\_swapped}, and \texttt{aesop\_core}; singleton controls are drawn from the same predeclared action subset. On the combined 432 replayable holdout goals, K=3 and K=4 solve 67 goals, compared with 54 for the best singleton and 30 for empty Hammer. The tested-action oracle is 68, so the frozen portfolio recovers nearly all available headroom without retuning on the holdout folds.

\begin{table}[H]
\caption{Frozen fresh-holdout validation with retrieved-only candidates. The action subset and portfolio order are fixed before holdout evaluation; the oracle is over the predeclared tested actions, not over the full action grid.}
...
\midrule
Fold 0 & 208 & 16 & 23 & 24 & 24 & \best{28} & \best{28} & 28 & 12/12 \\
Fold 1 & 224 & 14 & 31 & 32 & 34 & 39 & 39 & \best{40} & 25/26 \\
\rowcolor{teal!7}
Combined & 432 & 30 & 54 & 56 & 58 & \best{67} & \best{67} & 68 & \best{37/38} \\
\bottomrule
\end{tabular}}
\end{table}
