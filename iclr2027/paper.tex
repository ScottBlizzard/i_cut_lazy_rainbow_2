...
\begin{tabular}{lccc}
\toprule
Setting & Single interface & Small typed portfolio & Tested oracle \\
\midrule
Mixed-source 230 & 38 & \best{57} & 58 \\
Retrieved-only 230 & 35 & \best{52} & 52 \\
Fresh holdout 432 & 54 & \best{67} & 68 \\
\bottomrule
\end{tabular}}
\caption{Result ladder. Across the mechanism matrix, deployability anchor, and frozen holdout, most tested oracle headroom is recovered by a small typed portfolio rather than by the strongest single interface.}
...
\texttt{solve\_by\_elim}-only & 34/230 & 5/29 \\
...
The unsolved goals are also informative because they are partitioned by a verified intervention protocol. A miss after empty Hammer and after every single typed action is different from a miss after the full portfolio; the former points to candidate quality, while the latter points to missing proof actions, longer reasoning chains, or backend search limits. This is why the paper reports typed oracles, homogeneous controls, strict-goal filters, and retrieved-only anchors together. They turn a raw failure count into a map of where the prover stack should improve next. In that sense, \tap{} is useful even when it does not solve a goal: it makes the failure mode more local than an untyped premise-ranking metric can.

\paragraph{What a stronger next system should do.}
The natural next step is to learn an allocator that beats the fixed typed portfolio while preserving the same accounting discipline: same first attempt, same retry budget, same verified-outcome metric, and the same strong static controls. The present experiments make that target concrete. They identify where the oracle headroom is, show that a simple typed sequence captures most of it, and separate the compiler effect from both traced proof-core assistance and post-hoc portfolio fitting.
