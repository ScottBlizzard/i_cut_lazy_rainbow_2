...
\begin{abstract}
Premise selection is usually treated as the problem of ranking which lemmas to send to a Lean prover. This view is incomplete: after names are selected, they must still be compiled into the proof interface that can consume them. We study \emph{action-conditional evidence allocation}, where a method chooses not only selected names but also whether they enter Lean as Hammer facts, HammerCore inputs, simp lemmas, Aesop facts, Aesop simp rules, or elimination facts. We introduce \tap, a verified typed evidence compiler evaluated in a Mathlib 4.30 + LeanHammer 4.30 pre-theorem replay harness. On 230 replayable traced-corpus goals, the typed action grid reaches 58/230 kernel-verified proofs versus 38/230 for the best single action; a four-retry fixed portfolio reaches 57/230 out of fold and 58/230 when train-fitted. Removing traced proof-core names gives a retrieved-only anchor: empty Hammer solves 29/230, the best standalone action solves 35/230, and a fixed typed portfolio reaches the 52/230 typed oracle out of fold with three retry actions. Exact Aesop controls sharpen the mechanism: oracle-core plus retrieved joint exposure solves 38/230 goals, but identity exposure solves 36/230, simps-only solves 35/230, retrieved-only joint exposure solves 34/230, and oracle-core-only joint exposure solves 35/230. Thus the robust claim is not a pure facts-versus-simps complementarity; selected evidence has tactic-interface and source-dependent behavior that fixed typed portfolios can exploit. Stronger logistic and ComplementNB allocators do not beat or compress the fixed control, so adaptive routing is a boundary rather than a claim.
\end{abstract}
...
The main verified result is on non-generated replayable Mathlib theorem contexts. We migrate traced goals to a Mathlib 4.30 + LeanHammer 4.30 pre-theorem replay harness and identify 230 replayable goals from 490 cleaned traces. On this subset, the typed action grid reaches a 58/230 oracle, compared with 38/230 for the best single action. A fixed portfolio of four typed retry actions after \texttt{hammer\_empty} reaches 57/230 out of fold and 58/230 when train-fitted. To separate this mechanism from traced proof-core assistance, we also run a retrieved-only anchor that removes oracle-core names from the candidate source. There, empty Hammer solves 29/230, the best standalone action solves 35/230, and a fixed typed portfolio reaches the 52/230 typed oracle out of fold with three retry actions. The fixed sequence is not the proposed novelty; it is the hard matched Lean-call control that shows how large the interface-typing effect is. Stronger logistic and ComplementNB allocators match but do not beat the fixed control, so adaptive routing remains a future target.

This paper makes three contributions. First, we formulate action-conditional evidence allocation as a distinct problem from premise retrieval: selected evidence must be compiled into a Lean interface, not only ranked. Second, we implement and evaluate \tap{} in a real Mathlib 4.30 + LeanHammer 4.30 replay harness, showing that small typed action sets recover nearly all current oracle headroom on the replayable subset, including under a retrieved-only candidate source. Third, we provide mechanism evidence and boundaries: Aesop is sensitive to source and exposure choice, broad 32-name insertion is harmful in the original matrix, strict syntactic filtering does not explain the gains, and learned allocators do not yet beat the fixed typed control. Trace-core failure-feedback experiments remain important discovery evidence, but the current supported main claim is action-conditional evidence allocation.

The scope is deliberately narrower than a full theorem-proving leaderboard. We treat fixed typed portfolios as strong matched Lean-call controls, not as weak baselines, and we do not claim adaptive routing until it beats them. The retrieved-only anchor tests the downstream compiler once names are proposed, but it is not an official reproduction of LeanSearch v2 or another complete premise-selection system. We also distinguish bridge replay from open-ended proof generation. These boundaries keep the paper's claim reviewer-checkable: the contribution is a typed interface mechanism for selected evidence in Lean proof search.
...
\begin{table}[t]
\caption{Main claim-evidence map. The primary evidence is the verified typed action matrix, retrieved-only anchor, and Aesop channel counterfactual; trace-core and bridge rows are supporting discovery evidence.}
...
Retrieved-only compilation works & Empty Hammer: 29/230; best standalone: 35/230 & Fixed K=3/K=4 OOF: \best{52/230} & reaches typed oracle \\
...
Table~\ref{tab:retrieved-anchor} repeats the budgeted compiler test after removing traced proof-core names from the candidate source. This is the main deployability anchor for the paper: it does not reproduce a full external retriever, but it shows that once a retriever-like candidate list is fixed, typed Lean-interface allocation adds verified proofs under the same outer retry protocol.

\begin{table}[t]
\caption{Retrieved-only anchor after \texttt{hammer\_empty}. The fixed typed portfolio reaches the retrieved-only typed oracle out of fold with three retry actions.}
\label{tab:retrieved-anchor}
\centering
\small
\begin{tabular}{rrrr}
\toprule
...
\rowcolor{teal!7}
3 & \best{52/230} & \best{23/23} & 52/230 \\
4 & \best{52/230} & \best{23/23} & 52/230 \\
\bottomrule
\end{tabular}
\end{table}
