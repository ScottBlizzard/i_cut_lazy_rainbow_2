"""Probe pre-theorem patch replay inside original Mathlib 4.30 files."""
...
def insert_hammer_import(lines: list[str]) -> list[str]:
    ...
    for i, line in enumerate(lines):
        if line.startswith("import "):
            ...
            break
    if any(line.strip() == "import Hammer" for line in lines[: last_import + 2]):
        return lines
    out = list(lines)
    out.insert(last_import + 1, "import Hammer\n")
    return out
...
def run_one(
    *,
    ...
    out_file = save_dir / f"pretheorem_patch_{idx:04d}.lean"
    ...
    return {
        **base,
        ...
        "status": "proved" if result["success"] else classify(result["output_tail"]),
...
def write_md(payload: dict[str, Any], out: Path) -> None:
    lines: list[str] = []
    lines.append("# Mathlib 4.30 Pre-Theorem Patch Probe")
    lines.append("")
    lines.append(f"- Verdict: `{payload['verdict']}`")
    lines.append(f"- Input: `{payload['input_jsonl']}`")
    lines.append(f"- Goals checked: {payload['summary']['n_goals']}")
    lines.append(f"- Verified by patched Hammer proof: {payload['summary']['n_verified']}")
    lines.append("")
    ...
    lines.append("")
    lines.append("- This is the first probe of original-file/pre-theorem replay.")
    lines.append("- Failure can mean span drift, Hammer search weakness under the controlled options, or true premise insufficiency.")
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    payload = {
        "experiment": "mathlib430_pretheorem_patch_probe",
        "input_jsonl": str(input_jsonl),
        "mathlib_root": str(mathlib_root),
        "hammer_root": str(hammer_root),
        "save_dir": str(save_dir),
        "limit": args.limit,
        "verdict": "pass" if n_verified else "probe_completed_no_proofs",
        "summary": {
            "n_goals": len(results),
